...
### 5.1 Compiler unit tests
  ...
  - ✅ `union input compiles generated summaries` — now mirrors the record coverage: we compile/reflect both binaries and assert `ShapeProvided.MapParameters`/`Value` produce the expected strings, confirming union metadata also survives relocation.
    - ✅ `GenericInput_multipleInstantiations` — the tast-reflection typar scope fix (11 Nov @ 22:15 UTC) means the regression now finishes in ~3 s even under `timeout 300s env FS1023_TRACE=1 FS1023_KEEP_TEMP=1 dotnet test …`. The fresh run preserved `/var/folders/.../fs1023-967b455cca084103b9723efb6e521802/`, `/tmp/fs1023_provider.log`, and `/tmp/fs1023_trace.log`, each confirming that both `ProvidedGenericInt` and `ProvidedGenericString` make it through `methods-begin`/`methods-end` without recursion. The test now also reflects over the generated types to assert the summary properties (`MapParameters`, `OptionalParameter`, etc.) expose the expected strings, so we catch future metadata regressions automatically. (Historical context for the original hang remains below for posterity.)
    ...
    - **MSBuild thread dump:** re-running `dotnet-dump analyze /private/tmp/fs1023_generic_dump.dmp` today confirms the earlier diagnosis: thread 0 (main) is blocked in `Microsoft.Build.Execution.BuildSubmission.Execute`, threads 25–26 sit in `NodeProviderOutOfProcBase+NodeContext.DrainPacketQueue`, and threads 27/28/34 (OSIDs `0x1a34f1d`, `0x1a34f1e`, …) are stuck inside `Microsoft.Build.Tasks.Copy.ParallelCopyTask` → `WaitHandle.WaitOne`. That gives us concrete thread IDs to correlate with future MSBuild instrumentation.
    ...
    - **Next debugging hop:**
        1. Mine the captured dump/trace to identify which MSBuild request (target + path) owns the stuck `ParallelCopyTask`, then compare against the provider log to ensure there aren’t any remaining copy/dependency races (even though the test now completes, we still want to understand the earlier MSBuild waits).
