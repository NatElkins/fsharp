...
### 5.1 Compiler unit tests
  ...
  - ✅ `union input compiles generated summaries` — now mirrors the record coverage: we compile/reflect both binaries and assert `ShapeProvided.MapParameters`/`Value` produce the expected strings, confirming union metadata also survives relocation.
    ...
    - **Next debugging hop:**
        1. Re-run the generic regression (or the CLI `fsc @consumer.rsp`) under `timeout 300s` so the updated provider logs show the precise stage that hangs; pipe stdout/stderr to the preserved `generic.log` for inspection.
        2. Extend the watchdog harness to capture a `dotnet-trace`/`dotnet-dump` automatically when the timeout triggers so we can match the stuck provider stage with a managed stack (MSBuild node vs. provider reflection).
        3. If the provider logs prove we return cleanly, focus back on the hosting layer by injecting logging/stack capture right before `CompileHelpers.tryCompile` and inside `runCompileWithChecker`, then compare against a manual CLI run that references the same DLL set (fixing the lingering `ParamArray` attribution so the CLI build can succeed).
