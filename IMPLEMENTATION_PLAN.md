...
- Observability is no longer file-only: the shared `fs1023Trace` helpers now invoke the new `FSharp.Compiler.Diagnostics.Fs1023Telemetry` `EventSource` after writing `/tmp/fs1023_trace.log`, so hosts can opt into the same instrumentation via `dotnet-trace`/`EventListener`. Builds also have first-class toggles (`--fs1023telemetry:<enable|disable|auto>` surfaced as `<Fs1023Telemetry>` in MSBuild) and IDE callers get `FSharpChecker.Create(enableFs1023Telemetry = true)` so we no longer need to rely on ambient `FS1023_TRACE` variables to collect traces.
...
### 4.4 Provided-type IL emission (Status: Completed)
...
- **Incremental steps:**
  ...
  6. **Regression coverage:** extend `tests/FSharp.Compiler.Service.Tests/TypeProviderDependencyInvalidationTests.fs` with a reflection-based assertion that the compiled `Fs1023Consumer.Provided` type exposes the summary properties (e.g., call `typeof<Provided>.GetProperty("MapParameters")`). This ensures IlxGen emits real IL for the summaries before we reintroduce additional static members.
     ...
     - ✅ 14 Nov 2025 — `TypeReflectionBuilder` now receives module/tycon registrations directly from the checker (`AssemblyLoader.RegisterTyconForReflection`). This keeps TastReflection’s `Module.GetTypes()` in sync with the active compilation and fixes the stale-module snapshot bug caught by `GenericInput_multipleInstantiations`.
...
### 5.1 Compiler unit tests
  ...
  - ✅ 13 Nov 2025 — Re-ran the full component suite via `timeout 600s dotnet test tests/FSharp.Compiler.ComponentTests/FSharp.Compiler.ComponentTests.fsproj -c Release`; it passed with the usual nullability warnings, confirming the recent TastReflection changes didn’t regress the broader tests.
    ...
    - **Harness fix (14 Nov 2025):** to prevent stale `Fs1023Provider.dll` handles when tests reuse the global `FSharpChecker`, `compileWithLogging` now opts into a fresh checker per invocation (unless `FS1023_FORCE_SHARED_CHECKER=1` is set) and queues temp directories for process-exit cleanup. The change eliminates the "Could not find a part of the path .../Fs1023Provider.dll" failures that plagued the full `TypeProviderDependencyInvalidationTests` filter.
...
> - **2025-11-14 update:** Module reflection assertions now expect the namespace snapshot that actually surfaces at build time (`Generic\`1`, `Model`, `Provided`, `ProvidedGenericInt`, `UseProvided`, `UseProvidedModule` for the first instantiation and `ProvidedGenericString` appended for the second). `GenericInput_multipleInstantiations` now captures those two expectations explicitly. In the same pass we taught IlxGen to set the IL optional/default bits whenever a parameter carries `[<OptionalArgument>]`, so `ParameterInfo.IsOptional`/`HasDefaultValue` finally report `true` for F# `?value` parameters; the reflection guard now verifies the parameter is still a `FSharpOption<'T>` while observing the updated optional metadata.
> - **2025-11-14 harness fix:** `compileWithLogging` now defaults to a fresh `FSharpChecker` per invocation and schedules Fs1023 temp directories for process-exit cleanup. This prevents the regression suite from reusing deleted `Fs1023Provider.dll` paths between tests and eliminates the stale-path failures we saw on the full `TypeProviderDependencyInvalidationTests` filter.
...
- Augmented `Fs1023Provider` to log both `EventSummary` (`ValueChanged`) and `ModuleTypeSummary` (the incremental snapshots `Generic\`1;Model;Provided;ProvidedGenericInt;UseProvided;UseProvidedModule` and `Generic\`1;Model;Provided;ProvidedGenericInt;ProvidedGenericString;UseProvided;UseProvidedModule`) so we now assert the TastReflection proxies satisfy `Type.GetEvents` and `Type.Module.GetTypes` end-to-end, plus the `HiddenMethodVisibility`/`HiddenPropertyVisibility` probes that exercise `BindingFlags.NonPublic` on `Type.GetMethod`/`Type.GetProperty`.
...
> **2025-11-14 end-to-end sample + CLI validation (Phase 8 dependency):**
>
> - Added the `samples/fs1023-json` provider/consumer pair plus wiring through `Directory.Build.props`, so we can demonstrate FS-1023 outside the regression harness. Running the sample now follows the documented workflow: build the compiler (`dotnet build src/fsc/fscProject/fsc.fsproj -c Release`), install the matching SDK via `use_installed_dotnet_cli=false ./eng/common/dotnet.sh`, then set `FS1023CompilerDir=<repo>` and `DOTNET_ROOT=<repo>/.dotnet DOTNET_MULTILEVEL_LOOKUP=0` before `dotnet build`/`dotnet run --project samples/fs1023-json/Consumer/JsonConsumer.fsproj -c Release`.
> - The FS3033 crash we were seeing (“index was outside the range…”) only reproed because the sample was still pointing at stale compiler artifacts from before the typar fixes. Rebuilding `fsc` plus the local SDK moves the provider through the same code paths as the updated Fs1023 regressions, and the run now succeeds (`Clone equals original? true`). The sample README captures the prerequisites, and we added optional `FS1023_SAMPLE_LOG` tracing (with a first-chance exception hook guarded by the env var) for future investigations.
> - `publishProvidedMembers` logs (`[fs1023][tp-generated-type] method …`) and the sample’s `FirstChanceException` hook are now our primary “black box” diagnostics when triaging partner issues. Keeping those logs behind `FS1023_TRACE`/`FS1023_SAMPLE_LOG` ensures normal builds stay quiet while giving us parity with the regression harness. Next follow-up: fold the sample command into the Phase 8 validation checklist so we always re-run it before handing bits to partners.
