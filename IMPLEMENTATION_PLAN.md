...
### 5.1 Compiler unit tests
  ...
  - ✅ `union input compiles generated summaries` — now mirrors the record coverage: we compile/reflect both binaries and assert `ShapeProvided.MapParameters`/`Value` produce the expected strings, confirming union metadata also survives relocation.
    ...
    - **Timeout repro (done):** wrapped the test in a 5-minute watchdog via `python` + `subprocess (timeout=300)`; when it hung we captured a system sample (`/tmp/fs1023_generic_sample.txt`), the combined log (`/tmp/fs1023_generic_timeout.log`), a 30-second `dotnet-trace` session (`/tmp/fs1023_generic_trace.nettrace` + `.speedscope.json`), and a full `dotnet-dump` (`/tmp/fs1023_generic_dump.dmp`). All artifacts show the provider-side MSBuild submission stuck in `Microsoft.Build.Execution.BuildSubmission.Execute` → `WaitHandle.WaitOne`, so `FSharpChecker.Compile` never finishes producing `Fs1023Provider.dll` (the consumer compile never even starts).
    ...
    - **Next debugging hop:** mine the captured dump/trace for the exact MSBuild task pair that is waiting (e.g., inspect `/tmp/fs1023_generic_dump.dmp` with `dotnet-dump analyze` to enumerate `Microsoft.Build.BackEnd` threads and look for a `TaskHost` deadlock), and add checker-level tracing around the MSBuild submission so we log when each `BuildRequestData` starts/completes. Also consider enabling MSBuild binary logging (`/bl`) for the provider project to see which targets were last invoked.
