...
### 5.1 Compiler unit tests
  ...
  - ✅ `union input compiles generated summaries` — now mirrors the record coverage: we compile/reflect both binaries and assert `ShapeProvided.MapParameters`/`Value` produce the expected strings, confirming union metadata also survives relocation.
    - ⏳ `GenericInput_multipleInstantiations` — still skipped, but the latest forced run (`timeout 600s dotnet test ... --filter "GenericInput_multipleInstantiations"`) confirms `compileWithLogging` never logs `end generic-consumer`, i.e., `checker.Compile` hangs while compiling the consumer after the provider build succeeds. `generic.log` plus `FS1023_TRACE` in `/var/folders/.../fs1023-960b1e1a32594b26878cf027bde386f0/` capture the exact point of suspension (static-argument reflection completes in ~9 ms, then no further trace entries). Next debugging hop: sample the stuck process (SIGQUIT/dotnet-trace) or instrument the consumer compilation path just after `checker.Compile` kicks off so we know which phase never returns.
    - **Timeout repro (done):** wrapped the test in a 5-minute watchdog via `python` + `subprocess (timeout=300)`; when it hung we captured a system sample (`/tmp/fs1023_generic_sample.txt`) and the consolidated log (`/tmp/fs1023_generic_timeout.log`). The log shows `compileWithLogging` never reaches `end generic-provider`, so the hang is actually inside the provider compilation path triggered by `FSharpChecker.Compile` (the consumer compile never starts).
    - **Manual `fsc` repro (done):** running the recorded command from `generic.log` via `dotnet artifacts/bin/fsc/Release/net10.0/fsc.dll ...` inside the preserved temp directory completes immediately, which tells us the raw compiler toolchain is fine. The consumer command fails fast with a missing `FSharp.TypeProviders.SDK` reference rather than hanging. That narrows the problem to the FCS hosting surface (checker.IncrementalBuilder/TP reload), not to `fsc` itself.
    - **Next debugging hop:** inspect `/tmp/fs1023_generic_sample.txt` for the managed stack (the sample shows the main thread deep inside CoreCLR), add even more granular logging around the call into `checker.Compile`, and consider grabbing a `dotnet-trace` or `dotnet-dump` so we can see which async job never completes.
    - **Next debugging steps:** (1) capture a stack trace while the hang reproduces (SIGQUIT or `dotnet-trace`/`dotnet-dump collect` on the hung `dotnet test`), (2) add coarse logging around `compileWithLogging` so the log shows whether `ParseAndCheckProject` vs. `Compile` is stalling, and (3) reproduce manually by running the preserved temp project (`/var/folders/.../fs1023-960b1e1a32594b26878cf027bde386f0`) via `dotnet fsc`/`dotnet build` outside xUnit so we can attach profilers more easily.
