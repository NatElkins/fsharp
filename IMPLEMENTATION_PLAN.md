...
- **Next breakdown (Phase 4.4 remaining work):**
  ...
  4. **Wire `skipProviderStaticLinking`** — ✅ the new `MarkProvidedTypeIlEmitted` hook flips `TcImports.SkipProviderStaticLinking` only after `GenProvidedTypeDef` successfully adds the IL type, and the `FS1023_TRACE` log now records both the `record-tycon` and `il-emitted` events so we can confirm when relocation is bypassed.
