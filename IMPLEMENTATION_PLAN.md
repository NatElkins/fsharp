...
- **Next breakdown (Phase 4.4 remaining work):**
  ...
  2. **Emit methods/ctors (`GenProvidedTypeDef`)** — ⚙️ `genProvidedMemberBinding` now consumes the catalog entries directly, so we emit provided methods (and constructors) in declaration order, reusing the cached `ProvidedMemberBinding` bodies when present and falling back to `ReflectedDefinition` only when the binding did not round-trip. Remaining work: plumb ctor-specific tracing and capture the resulting `ILMethodDef`s so we can hang property/event shells on top.
