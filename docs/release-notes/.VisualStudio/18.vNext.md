...
* Prototype F# expression evaluator for the debugger: a Concord `IDkmClrExpressionCompiler` registered for the F# language id (`FSharp.ExpressionEvaluator`), so F# frames no longer fall back to the C# evaluator. The prototype serves the Locals window from the frame's arguments and PDB locals and evaluates integer literals; F# expressions follow. ([Issue #2544](https://github.com/dotnet/fsharp/issues/2544), [Issue #12812](https://github.com/dotnet/fsharp/issues/12812))
