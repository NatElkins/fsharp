...
- Observability now rides on `Fs1023Telemetry`, a lightweight `EventSource` we added under `FSharp.Compiler.Diagnostics`. Every `FS1023_TRACE` helper (service layer, background compiler, incremental builder, IlxGen, MethodCalls, CompilerImports) duplicates its file-backed log line into this EventSource, so IDE hosts can flip tracing on via `dotnet-trace`/`EventListener` without touching the file system. This gives Phase 8 a structured pipeline for telemetry/dogfood scenarios while keeping the legacy log-based workflow for ad-hoc investigations.
- Host toggles are wired up end-to-end: MSBuild exposes `<Fs1023Telemetry>` (passed through `--fs1023telemetry:<enable|disable|auto>`), and `FSharpChecker.Create(?enableFs1023Telemetry=...)` sets the same override for IDE callers. Both feed `Fs1023TraceControl`, so the EventSource/file logs respect CLI/MSBuild/IDE intent instead of only reading `FS1023_TRACE` from the environment.
...
- We now have a public-facing FS-1023 sample (`samples/fs1023-json/`) that mirrors the regression fixtures but runs under the normal SDK toolchain. The sample’s README calls out the prerequisites (`dotnet build src/fsc/fscProject/fsc.fsproj -c Release`, `use_installed_dotnet_cli=false ./eng/common/dotnet.sh`, `FS1023CompilerDir`, `DOTNET_ROOT=.dotnet`, `DOTNET_MULTILEVEL_LOOKUP=0`), and enabling `FS1023_SAMPLE_LOG` opts into the same trace payloads the regression harness produces (including the new `[fs1023][tp-generated-type] method …` breadcrumbs we added to `publishProvidedMembers`). This gives Phase 8 an end-to-end validation target and a repro harness we can share with partner providers without pointing them at the test infrastructure.
...
- 2025‑11‑13 status update: the Fs1023 “mutable provided type exposes setter and constructor” regression is now green end-to-end.
  - Reintroduced the `MutableProvider`/`MutableModel`/`MutableConsumer` fixtures with an ML-compatible provider that exposes a static `MutableSummary` property plus a parameterless constructor.
  - Extended `publishProvidedMembers` so it now registers both property setters (`GetSetMethod`) and concrete constructors (`GetConstructors`) in the typed tree, wiring them through `ProvidedMemberBindingHelpers.createFor{Method,Constructor}` so IlxGen can emit the cached invoker expressions without falling back to reflection.
  - Verified the typed tree for `MutableFs1023Consumer.Provided` now publishes `set_MutableSummary` and `.ctor`, and confirmed the runtime reflection checks succeed for both normal and `/standalone` builds via `dotnet test tests/FSharp.Compiler.Service.Tests/FSharp.Compiler.Service.Tests.fsproj -c Release --filter "DisplayName~mutable provided type exposes setter and constructor"`.
- 2025‑11‑13 follow-up: provided CLI events now flow through the same compiler pipeline.
- 2025‑11‑13 module reflection update: `ReflectModule` now proxies `System.Reflection.Module` for the projected assemblies.
  - The new proxy only overrides the virtual members exposed by the netstandard2.0 reference assemblies (`GetTypes`, `FindTypes`, `GetFields`, `GetMethods`, `GetMethodImpl`, `Resolve*`, etc.) and forwards them to the in-memory types surfaced by `ReflectAssembly`. APIs that are either non-virtual (`ModuleHandle`) or do not make sense for TastReflection (manifest-resource streaming, metadata token resolution, serialization) deliberately raise `NotSupportedException`, matching the story we documented for other unsupported reflection entrypoints.
  - While carving out the module proxy we re-threaded `ReflectAssembly`’s helpers (`TxTypeDef`, `TxTType`, `TryBindType`, typar-scope management, and tracing) so cache ownership stays within `ReflectAssembly`. `TypeReflectionBuilder` therefore still has a single place to track dependency scopes, and `ReflectModule` focuses purely on surfacing the consumer-facing reflection surface.
- Regression coverage now calls `sourceType.Module.GetTypes()` inside `Fs1023Provider` and asserts that the namespace-local types exposed through TastReflection match the compiler’s incremental view: the first instantiation (int) sees `Generic\`1`, `Model`, `Provided`, `ProvidedGenericInt`, `UseProvided`, `UseProvidedModule`, while the second instantiation (string) adds `ProvidedGenericString` to that set. The same regression also records any events exposed on the source type (`ValueChanged`) so we simultaneously exercise `Type.GetEvents` on the TastReflection proxy.
  - `ReflectTypeDefinition.GetConstructors`, `GetMethods`, `GetFields`, `GetProperties`, and `GetEvents` now combine declared members with provided augmentations (including `tcaug_provided_events`) and respect `BindingFlags` for visibility/scope. The regression fixture gained private members (`HiddenSummary`, `HiddenResult`) plus the `HiddenMethodVisibility`/`HiddenPropertyVisibility` summaries so we prove `BindingFlags.NonPublic` discovers them while the public lookup does not.
  - `ReflectModule` now exposes a debugger-friendly view (`DebuggerDisplay` + enriched `ToString`) that includes both the module identifier and the parent assembly name, keeping the “preview tooling” experience in sync with the architecture narrative.
  - `TyconAugmentation` captures per-tycon event metadata (handler type plus the synthesized `add_`/`remove_` members) so IlxGen can recreate the event definitions when emitting IL instead of depending on provider-supplied IL.
  - `publishProvidedMembers` plumbs `ProvidedEventInfo.Get{Add,Remove}Method` through `ProvidedMemberBindingHelpers.createForMethod`, registers the add/remove `Val`s in declaration order, and records the handler type so the IL generator can stamp the correct delegate reference.
  - `collectProvidedMembersForTycon`/`GenProvidedTypeDef` now build and emit `ILEventDef`s from that catalog, and the new regression (`provided event surfaces in emitted IL`) loads both the normal and `/standalone` consumers to assert the event metadata (`GetEvent("Triggered")`) and runtime semantics survive relocation.
...
- Provider constructors and property setters now flow through the same publication path, so `collectProvidedMembersForTycon` hands IlxGen complete metadata. The Fs1023 regression asserts `set_MutableSummary` and `.ctor` appear in the typed tree and reflects over both normal and `/standalone` binaries to prove the compiler-emitted setter/ctor function without consulting provider IL.
...
## Observability & telemetry (Phase 8 runway)

The original investigation tooling revolved around ad-hoc text files (`FS1023_TRACE=1` funnels logs into `/tmp/fs1023_trace.log`). That kept us unblocked while chasing hangs, but it does not scale to IDE dogfooding or to the preview telemetry we promised in Phase 8. We therefore introduced `FSharp.Compiler.Diagnostics.Fs1023Telemetry`, a minimal `EventSource` whose only event mirrors the existing trace string. Every `fs1023Trace`/`compileWithLogging` helper now calls into this EventSource after writing the file log, so:

1. Hosts can toggle tracing at runtime (`dotnet-trace collect --providers FSharpCompiler-FS1023`, VS `EventListener`, Ionide’s CLI) without touching the file system.
2. We can correlate compiler events with MSBuild/IDE telemetry via `ActivityId`.
3. Phase 8 “Observability & telemetry” work no longer needs to reinvent plumbing; we just need to add richer event IDs/payloads.

What we tried earlier: we experimented with `EventSource` directly inside `BackgroundCompiler` in early November, but emitting there alone missed IlxGen/service-layer activity and made trace reconstruction harder. The current design centralises the write in each existing helper, so every category (`[compileops]`, `[incrementalbuilder]`, `[ilxgen]`, `[typeproviders]`, etc.) automatically flows to both sinks with identical formatting. Remaining work (tracked in the implementation plan) is to (a) add explicit enable/disable flags surfaced through MSBuild properties/IDE options, (b) define the structured payloads we want to aggregate (counts vs. raw strings), and (c) document how partner teams capture traces alongside `fs1023`-specific binlogs.

With the latest iteration we have (a) in place: builds can set `<Fs1023Telemetry>` (or pass `--fs1023telemetry:<enable|disable|auto>` directly), and IDE hosts flip the same switch via `FSharpChecker.Create(enableFs1023Telemetry = true)`. Both code paths flow through `Fs1023TraceControl`, so the EventSource/file pipeline now respects MSBuild/IDE intent while still falling back to the `FS1023_TRACE` environment variable for one-off investigations. The remaining observability work is to define structured event payloads and publish the telemetry playbook.
