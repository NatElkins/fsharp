...
- Test coverage focus: record/union/C# scenarios and the component suite are green; the remaining blocker is the generic static-argument regression. Running it under a 5-minute watchdog now captures `/tmp/fs1023_generic_sample.txt` (via macOS `sample`) plus `/tmp/fs1023_generic_timeout.log`, and the enhanced `generic.log` shows `checker.Compile` never returns from the provider compile (consumer compile is never attempted). Replaying the exact `fsc` command recorded in the log succeeds immediately, which points to the FCS hosting path (incremental builder/type-provider reload) rather than the command-line compiler. IDE/TPSDK sweeps remain to-do once this hang and the IlxGen emission work are addressed.
