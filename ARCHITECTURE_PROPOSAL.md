...
- Test coverage focus: both the record and union regressions now pass (normal + `/standalone`), the component suite also runs cleanly, and the C# consumer harness targets `net10.0`, so cross-language consumers can execute generated members without the provider DLL. Remaining gaps are the skipped generic regression and the IDE/TPSDK sweeps once IlxGen emits IL for all generated members.
