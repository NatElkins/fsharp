...
- Static linking now only skips the provider relocation path once IlxGen has successfully emitted the concrete IL type: `MarkProvidedTypeIlEmitted` flips the `SkipProviderStaticLinking` flag and logs the transition under `FS1023_TRACE`, so `/standalone` builds continue to include provider IL whenever we fall back to the legacy pipeline.
