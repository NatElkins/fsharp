...
- Test coverage focus: record/union/C# scenarios and the component suite are green; the remaining blocker is the generic static-argument regression. Running it under a 5-minute watchdog now captures `/tmp/fs1023_generic_sample.txt`, `/tmp/fs1023_generic_trace.nettrace`, `/tmp/fs1023_generic_dump.dmp`, and `/tmp/fs1023_generic_timeout.log`, all of which show the provider-side MSBuild submission stuck in `Microsoft.Build.Execution.BuildSubmission.Execute` → `WaitHandle.WaitOne`. Replaying the logged command line via `dotnet artifacts/bin/fsc/Release/net10.0/fsc.dll …` finishes instantly, so the hang lives in the FCS/MSBuild hosting path (incremental builder/type-provider reload) rather than the CLI compiler. Next step is to mine the dump/trace for the specific MSBuild task deadlock and add checker-level logging around the build submission before re-enabling the test.
...
- `TastReflection.computeParameterMetadata` aligns with the CLR view of optional parameters: F#-style `?value` arguments no longer synthesize `HasDefaultValue = true`, `[<Optional>]` parameters still expose `ParameterAttributes.Optional`, and `ParameterInfo.IsDefined` only reports whichever attribute actually exists (`OptionalArgumentAttribute` vs. `OptionalAttribute`). The Fs1023 regression now observes `OptionalParameter = "value:false:false"` while `OptionalLiteralParameter` remains `value:true:true:42`.
