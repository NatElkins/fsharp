...
- 2025‑11‑13 module reflection update: `ReflectModule` now proxies `System.Reflection.Module` for the projected assemblies.
  ...
  - `ReflectTypeDefinition.GetConstructors` and `GetMethods` now combine declared members with provided augmentations and respect `BindingFlags` for visibility/scope. The regression fixture gained a private `HiddenSummary` method plus the `HiddenMethodVisibility` summary so we prove `BindingFlags.NonPublic` discovers the method while the public lookup does not.
