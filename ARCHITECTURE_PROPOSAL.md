...
- Test coverage focus: both the record and union regressions now pass (normal + `/standalone`), and the C# consumer harness targets `net10.0`, so cross-language consumers can execute generated members without the provider DLL. Remaining gaps are the skipped generic regression and the yet-to-be-run component/IDE suites once IlxGen emits IL for all generated members.
