...
- Test coverage focus: the record-based regression is passing again, but the union and C# consumer scenarios are still pending until IlxGen emits the remaining summary members and the C# harness targets the same `System.Runtime` version as the Fs1023 consumer.
