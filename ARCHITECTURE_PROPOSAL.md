...
- Test coverage focus: both the record and union regressions now pass (normal + `/standalone`), the component suite also runs cleanly, and the C# consumer harness targets `net10.0`, so cross-language consumers can execute generated members without the provider DLL. The outstanding gap is the skipped generic regression—our latest repro (killed after 600 s) shows the hang occurs inside `checker.Compile` for the consumer project (`generic.log` never emits “end generic-consumer,” while the provider build succeeds). IDE/TPSDK sweeps remain to-do once the generic hang and IlxGen emission are addressed.
