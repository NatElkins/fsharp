...
- 2025‑11‑13 module reflection update: `ReflectModule` now proxies `System.Reflection.Module` for the projected assemblies.
  ...
  - Regression coverage now calls `sourceType.Module.GetTypes()` inside `Fs1023Provider` and asserts that only the namespace-local types (`Model`, `Provided`, `UseProvided`) are reported. The same regression also records any events exposed on the source type (`ValueChanged`) so we simultaneously exercise `Type.GetEvents` on the TastReflection proxy.
