...
module NameMapTests =
    ...
    let private compileHotReloadLibrary source =
        FSharp source
        |> withOptions [ "--langversion:preview"; "--enable:hotreloaddeltas"; "--optimize-" ]
        |> asLibrary
        |> compile
        |> shouldSucceed
...
"""
        ...
        assertNoLineNumberSuffix helperNames
        Assert.True(helperNames |> List.exists (fun name -> name.Contains("@hotreload")), "Expected union helpers to use @hotreload naming.")
